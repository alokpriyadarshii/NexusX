...
from flask_nexusx.utils import http_status_message, unpack, OrderedDict
from flask_nexusx.representations.json import output_json
import sys
...
class Api(object):
    ...
    def _deferred_blueprint_init(self, setup_state):
        ...
        When a :class:`NexusX.Api` object is initialized with a blueprint,
        ...
        if not setup_state.first_registration:
            raise ValueError('NexusX blueprints can only be registered once.')
    ...
    def error_router(self, original_handler, e):
        """This function decides whether the error occured in a NexusX
        endpoint or not. If it happened in a NexusX endpoint, our
        handler will be dispatched. If it happened in an unrelated view, the
        app's original error handler will be dispatched.
        In the event that the error occurred in a NexusX endpoint but
    ...
    def handle_error(self, e):
        ...
        # Keep Werkzeug's generated Content-Length from being duplicated.
    ...
    def resource(self, *urls, **kwargs):
        """Wraps a :class:`~NexusX.Resource` class, adding it to the
        api. Parameters are the same as :meth:`~NexusX.Api.add_resource`.
    ...
    def mediatypes(self):
        ...
        for header in request.headers.get('Accept', '').split(','):
            mediatype = header.split(';', 1)[0].strip()
            if mediatype and mediatype not in seen:
                seen.add(mediatype)
                headers.append((mediatype, request.accept_mimetypes[mediatype]))
        return [h for h, q in sorted(headers,
    ...
    def unauthorized(self, response):
        ...
        if self.serve_challenge_on_401:
            realm = current_app.config.get("HTTP_BASIC_AUTH_REALM", "NexusX")
...
class Resource(MethodView):
    """
    Represents an abstract NexusX resource. Concrete resources should
    extend from this class and expose methods for each supported HTTP
    method. If a resource is invoked with an unsupported HTTP method,
    the API will return a response with status 405 Method Not Allowed.
    Otherwise the appropriate method is called and passed all arguments
    from the url rule used when adding the resource to an Api instance. See
    :meth:`~NexusX.Api.add_resource` for details.
    """
...
def marshal(data, fields, envelope=None):
    ...
    >>> from flask_nexusx import fields, marshal
    >>> data = { 'a': 100, 'b': 'foo' }
...
class marshal_with_field(object):
    ...
    >>> from flask_nexusx import marshal_with_field, fields
