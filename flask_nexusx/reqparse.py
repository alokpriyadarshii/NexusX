...
def _is_location_value(value):
    return hasattr(value, 'items') or hasattr(value, 'getlist') or isinstance(value, (list, tuple))


def _coerce_location_value(value):
    if value is None:
        return MultiDict()
    if type(value).__module__.startswith(('mock', 'unittest.mock')):
        return MultiDict()
    if not _is_location_value(value):
        return MultiDict()
    return value
...
def _get_json(request):
    ...
    if callable(get_json):
        try:
            ...
            data = None
        return _coerce_location_value(data)
    return _coerce_location_value(getattr(request, 'json', None))
...
class Argument(object):
        ...
        if isinstance(self.location, six.string_types):
            ...
            for l in self.location:
                ...
                if callable(value):
                    value = value()
                if value is not None:
                    values.update(_coerce_location_value(value))
            return values
    ...
    def handle_validation_error(self, error, bundle_errors):
        ...
        if current_app.config.get("BUNDLE_ERRORS", False) or bundle_errors:
            return error, msg
        flask_nexusx.abort(400, message=msg)
...
class RequestParser(object):
    ...
    single request. Ex::
        ...
        from flask_nexusx import reqparse
    ...
    def parse_args(self, req=None, strict=False, http_error_code=400):
        ...
        :param http_error_code: use custom error code for `NexusX.abort()`
        """
        if req is None:
            req = request
        ...
        if errors:
            flask_nexusx.abort(http_error_code, message=errors)
