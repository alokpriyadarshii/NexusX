...
from flask_nexusx.utils import http_status_message, unpack
import flask_nexusx
import flask_nexusx.fields
from flask_nexusx import OrderedDict
from json import dumps, loads, JSONEncoder
from tests.compat import assert_equal
import six
from types import SimpleNamespace
from unittest.mock import patch

_FLASK_NEXUSX_SYS_EXC_INFO = 'flask_nexusx.sys.exc_info'
...
class HelloWorld(flask_nexusx.Resource):
    def get(self):
        return {}
...
class HelloBomb(flask_nexusx.Resource):
    def get(self):
...
class APITestCase(unittest.TestCase):
    ...
    def test_unauthorized(self):
        app = Flask(__name__)
        api = flask_nexusx.Api(app, serve_challenge_on_401=True)
        response = Mock()
        response.headers = {}
        with app.test_request_context('/foo'):
            response = api.unauthorized(response)
    ...
    @patch(_FLASK_NEXUSX_SYS_EXC_INFO)
    ...
    def test_handle_error_does_not_swallow_custom_exceptions(self):
        ...
        api = flask_nexusx.Api(app, errors=errors)
    ...
    def test_handle_error_does_not_swallow_abort_response(self):
        ...
        class HelloBombAbort(flask_nexusx.Resource):
            def get(self):
    ...
    def test_marshal(self):
        fields = OrderedDict([('foo', flask_nexusx.fields.Raw)])
        marshal_dict = OrderedDict([('foo', 'bar'), ('bat', 'baz')])
        output = flask_nexusx.marshal(marshal_dict, fields)
        self.assertEqual(output, {'foo': 'bar'})

    def test_marshal_with_envelope(self):
        fields = OrderedDict([('foo', flask_nexusx.fields.Raw)])
        marshal_dict = OrderedDict([('foo', 'bar'), ('bat', 'baz')])
        output = flask_nexusx.marshal(marshal_dict, fields, envelope='hey')
        self.assertEqual(output, {'hey': {'foo': 'bar'}})

    def test_marshal_decorator(self):
        fields = OrderedDict([('foo', flask_nexusx.fields.Raw)])

        @flask_nexusx.marshal_with(fields)
        def try_me():
    ...
    def test_marshal_decorator_with_envelope(self):
        ...
        @flask_nexusx.marshal_with(fields, envelope='hey')
        def try_me():
            return OrderedDict([('foo', 'bar'), ('bat', 'baz')])
    ...
    def test_marshal_field_decorator(self):
        field = flask_nexusx.fields.Raw

        @flask_nexusx.marshal_with_field(field)
        def try_me():
            return 'foo'
    ...
    def test_marshal_field(self):
        ...
        output = flask_nexusx.marshal(marshal_fields, fields)
        self.assertEqual(output, {'foo': 'bar'})
    ...
    def test_marshal_tuple_with_envelope(self):
        fields = OrderedDict({'foo': flask_nexusx.fields.Raw})
        marshal_fields = OrderedDict([('foo', 'bar'), ('bat', 'baz')])
        output = flask_nexusx.marshal((marshal_fields,), fields, envelope='hey')
        self.assertEqual(output, {'hey': [{'foo': 'bar'}]})

    def test_marshal_nested(self):
        fields = OrderedDict([
            ('foo', flask_nexusx.fields.Raw),
            ('fee', flask_nexusx.fields.Nested({
                'fye': flask_nexusx.fields.String,
            }))
        ])
    ...
    def test_marshal_nested_with_non_null(self):
        fields = OrderedDict([
            ...
            ('fee', flask_nexusx.fields.Nested(
                OrderedDict([
                    ('fye', flask_nexusx.fields.String),
                    ('blah', flask_nexusx.fields.String)
                ]), allow_null=False))
        ])
    ...
    def test_marshal_nested_property(self):
        ...
        obj.foo = 'bar'
        obj.bat = 'baz'
        output = flask_nexusx.marshal([obj], fields)
        expected = [OrderedDict([('foo', 'bar'), ('fee', OrderedDict([('fye', None), ('blah', 'cool')]))])]
        self.assertEqual(output, expected)
    ...
    def test_marshal_list(self):
        fields = OrderedDict([
            ('foo', flask_nexusx.fields.Raw),
            ('fee', flask_nexusx.fields.List(flask_nexusx.fields.String))
        ])
        marshal_fields = OrderedDict([('foo', 'bar'), ('bat', 'baz'), ('fee', ['fye', 'fum'])])
        output = flask_nexusx.marshal(marshal_fields, fields)
    ...
    def test_marshal_list_of_nesteds(self):
        fields = OrderedDict([
            ('foo', flask_nexusx.fields.Raw),
            ('fee', flask_nexusx.fields.List(flask_nexusx.fields.Nested({
                'fye': flask_nexusx.fields.String
            })))
        ])
        marshal_fields = OrderedDict([('foo', 'bar'), ('bat', 'baz'), ('fee', {'fye': 'fum'})])
    ...
    def test_marshal_list_of_lists(self):
        fields = OrderedDict([
            ('foo', flask_nexusx.fields.Raw),
            ('fee', flask_nexusx.fields.List(flask_nexusx.fields.List(
                flask_nexusx.fields.String)))
        ])
        marshal_fields = OrderedDict([('foo', 'bar'), ('bat', 'baz'), ('fee', [['fye'], ['fum']])])
        output = flask_nexusx.marshal(marshal_fields, fields)
        expected = OrderedDict([('foo', 'bar'), ('fee', [['fye'], ['fum']])])
    ...
    def test_marshal_nested_dict(self):
        fields = OrderedDict([
            ('foo', flask_nexusx.fields.Raw),
            ('bar', OrderedDict([
                ('a', flask_nexusx.fields.Raw),
                ('b', flask_nexusx.fields.Raw),
            ])),
        ])
    ...
    def test_api_prefix(self):
        app = Mock()
        app.configure_mock(**{'record.side_effect': AttributeError})
        api = flask_nexusx.Api(app, prefix='/foo')
    ...
    def test_handle_api_error(self):
        ...
        class Test(flask_nexusx.Resource):
            def get(self):
                flask.abort(404)
    ...
    def test_non_api_error_404_catchall(self):
        app = Flask(__name__)
        api = flask_nexusx.Api(app, catch_all_404s=True)
        app = app.test_client()
    ...
    def test_error_router_falls_back_to_original(self):
        """Verify that if an exception occurs in the NexusX error handler,
    ...
    def test_add_two_conflicting_resources_on_same_endpoint(self):
        ...
        class Foo1(flask_nexusx.Resource):
            def get(self):
                return 'foo1'

        class Foo2(flask_nexusx.Resource):
            def get(self):
                return 'foo2'
    ...
    def test_resource(self):
        app = Flask(__name__)
        resource = flask_nexusx.Resource()
        resource.get = Mock()
        with app.test_request_context("/foo"):
    ...
    def test_abort_data(self):
        try:
            flask_nexusx.abort(404, foo='bar')
    ...
    def test_abort_custom_message(self):
        try:
            flask_nexusx.abort(404, message="no user")
            assert False  # We should never get here
        except Exception as e:
            self.assertEqual(e.data['message'], "no user")

    def test_abort_type(self):
        self.assertRaises(HTTPException, lambda: flask_nexusx.abort(404))
    ...
    def test_exception_header_forwarded(self):
        ...
        app.aborter.mapping.update({304: NotModified})
    ...
    def test_exception_header_forwarding_doesnt_duplicate_headers(self):
        ...
        This covers the duplicate Content-Length regression.
        """
        app = Flask(__name__)
        api = flask_nexusx.Api(app)
    ...
    def test_use_custom_jsonencoder(self):
        ...
        class TestConfig(object):
            FLASK_NEXUSX_JSON = {'cls': CabageEncoder}

        app = Flask(__name__)
        app.config.from_object(TestConfig)
        api = flask_nexusx.Api(app)

        class Cabbage(flask_nexusx.Resource):
            def get(self):
                return {'frob': object()}
    ...
    def test_redirect(self):
        ...
        class FooResource(flask_nexusx.Resource):
            def get(self):
                return redirect('/')
    ...
    def test_json_float_marshalled(self):
        ...
        class FooResource(flask_nexusx.Resource):
            fields = {'foo': flask_nexusx.fields.Float}
            def get(self):
                return flask_nexusx.marshal({"foo": 3.0}, self.fields)
    ...
    def test_selectively_apply_method_decorators(self):
        ...
        class TestResource(flask_nexusx.Resource):
